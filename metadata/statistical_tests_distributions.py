...
def main():
    ...
    for patient in os.listdir(OSTEOPENIA_DATASET):
        ...
        for patient_lbl in sorted_lbls:
            ...
            data.append(
                [patient_lbl['osteopenia'], patient_lbl['gender'], patient_lbl['study_number'], patient_lbl['age'],
    ...
    patients_df = pd.DataFrame(data, columns=["osteopenia", 'gender', "study_number", "age", "initial_exam",
                                              "fracture_visible", "metal", "cast"])
    ...
    print("[Osteopenia vs gender]", cramers_v(cm_osteopenia_gender), ", p-value",
          ss.chi2_contingency(cm_osteopenia_gender)[1])  # none
    cm_osteopenia_fracture = pd.crosstab(patients_df['osteopenia'], patients_df['fracture_visible'])
    print("[Osteopenia vs fracture_visible]", cramers_v(cm_osteopenia_fracture), ", p-value",
          ss.chi2_contingency(cm_osteopenia_fracture)[1])  # none
    cm_osteopenia_metal = pd.crosstab(patients_df['osteopenia'], patients_df['metal'])
    print("[Osteopenia vs metal]", cramers_v(cm_osteopenia_metal), ", p-value",
          ss.chi2_contingency(cm_osteopenia_metal)[1])  # none
    cm_osteopenia_initial_exam = pd.crosstab(patients_df['osteopenia'], patients_df['initial_exam'])
    print("[Osteopenia vs initial_exam]", cramers_v(cm_osteopenia_initial_exam), ", p-value",
          ss.chi2_contingency(cm_osteopenia_initial_exam)[1])  # small-to-medium, significant
    cm_osteopenia_cast = pd.crosstab(patients_df['osteopenia'], patients_df['cast'])
    print("[Osteopenia vs cast]", cramers_v(cm_osteopenia_initial_exam), ", p-value",
          ss.chi2_contingency(cm_osteopenia_cast)[1])  # small-to-medium, significant

    print("\n")
    # point biserial [-1, 1]
    # categorical-continous: osteopenia, study_number. age
    print("--------------------------- Categorical - continous variable relationship ---------------------------")
    print("[Osteopenia vs study_number]",
          ss.pointbiserialr(patients_df['osteopenia'], patients_df['study_number']))  # significant positive correlation
    ...
    patients_without_osteopenia_diag = sorted(patients_without_osteopenia_diag.items(), key=lambda x: x[1],
                                              reverse=True)
    ...
    patients_without_osteopenia_count_frac = sorted(patients_without_osteopenia_count_frac.items(), key=lambda x: x[1],
                                                    reverse=True)
    plt.figure(figsize=(8, 8))
